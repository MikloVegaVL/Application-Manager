...
def render_html(url: str, timeout: float) -> str | None:
    ...
    with playwright_launch_semaphore:
        try:
            ...
            logger.warning("Playwright-Rendering von %s fehlgeschlagen: %s", url, exc)
            return None
...
def fetch_with_requests(url: str, timeout: float = 15.0) -> str | None:
    ...
    try:
        ...
        logger.warning("Abruf von %s fehlgeschlagen: %s", url, exc)
        return None
...
def extract_offers(
    ...
    *,
    ...
    """Zweistufige Extraktion: zuerst JSON-LD, dann Heuristik."""
    soup = BeautifulSoup(html, "html.parser")

    offers = extract_json_ld_offers(soup, source_url, source_platform)
    if offers:
        return offers
