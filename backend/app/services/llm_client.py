...
Bündelt an einer Stelle (KTD6 im Migrationsplan), was sowohl `pdf_parser.py`
als auch `ai_generator.py` brauchen:
...
3. Eine Retry-Schleife: Bei einem Validierungsfehler wird die ungültige
   ...
   und einmal erneut versucht (A2 im Plan).
4. Einen Fallback für Ollamas bekannten `$defs`/`$ref`-Bug bei verschachtelten
   Listen-Feldern (ollama/ollama#8444, KTD5): Schlägt der Retry erneut auf
...
sind ausschließlich helferintern sichtbar (KTD4) - Aufrufer (`pdf_parser.py`,
...
# Submodell-Feld (`name -> (Original-Modelklasse, abgeflachte Modelklasse,
# dessen eigene Listenfelder, dessen eigene Submodell-Felder)`).
_ListFieldMap = dict[str, type[BaseModel]]
_SubmodelMap = dict[str, tuple[type[BaseModel], type[BaseModel], "_ListFieldMap", "_SubmodelMap"]]
...
def _call_chat(
    ...
    Fängt genau die drei laut KTD3 relevanten Fehlerarten ab -
    `ollama.ResponseError`, das eingebaute `ConnectionError` (in das Ollamas
    Client `httpx.ConnectError` unverpackt umwandelt) sowie
    `httpx.TimeoutException` (von Ollamas Client NICHT abgefangen, muss
    separat behandelt werden) - und wirft dafür einheitlich
    `LlmUnavailableError`, ohne erneuten Versuch (Retry gilt laut KTD3/AE3
    nur für Validierungsfehler, nicht für Nichterreichbarkeit).
    """
    try:
...
def _nested_list_field_paths(
    ...
    Submodell-Feld erreichbar) - das ist genau die Feldklasse, die laut
    KTD5 vom Ollama-`$defs`-Bug betroffen sein kann."""
...
def _is_structural_failure(
    ...
    """KTD5s "strukturelles Signal": Der Retry (der den ersten
    Validierungsfehler bereits im Prompt enthielt) scheitert erneut am
    selben verschachtelten Listenfeld wie der Erstversuch - z. B.
    `experiences`/`education` oder deren Äquivalent im CV-Content-Schema.
    Ein Fehler auf einem anderen oder einem Top-Level-Feld beim zweiten
    Versuch ist KEIN strukturelles Signal, sondern gewöhnliche
    Retry-Erschöpfung (R7/AE2)."""
...
def _build_flat_variant(
    ...
    die von Ollamas `$defs`/`$ref`-Bug (KTD5) betroffen sein könnte.
    ...
    in die ursprüngliche verschachtelte Form zurückzubauen.
    """
    ...
    for name, field in model_cls.model_fields.items():
        ...
        if isinstance(annotation, type) and issubclass(annotation, BaseModel):
            ...
            submodels[name] = (annotation, flat_sub, sub_list_fields, sub_submodels)
...
def _reconstruct(
    ...
    for name in model_cls.model_fields:
        if name in list_fields:
            ...
            orig_sub, _flat_sub, sub_list_fields, sub_submodels = submodels[name]
...
def _retry_messages(base_messages: list[Message], invalid_content: str, error: ValidationError) -> list[Message]:
    """A2: Baut den Retry-Prompt - die ungültige Antwort plus der konkrete
...
def _fallback_messages(base_messages: list[Message]) -> list[Message]:
    """KTD5-Fallback-Prompt: weist auf das abgeflachte Ersatzschema hin, in
    dem Listenfelder als JSON-kodierte Strings statt als Arrays erwartet
    werden."""
    return base_messages + [
        {
            "role": "user",
...
def generate_structured(
    ...
    *,
    ...
    2. Bei Validierungsfehler: ein Retry, dessen Prompt die ungültige
       Antwort und den konkreten Validierungsfehler enthält (A2).
    3. Scheitert auch der Retry, und betrifft der Fehler in beiden
       Versuchen dasselbe verschachtelte Listenfeld (KTD5s "strukturelles"
       Signal), ein letzter Versuch gegen ein abgeflachtes Schema, dessen
       Ergebnis in die ursprüngliche verschachtelte Form zurückgebaut wird.
    ...
    (KTD4) - Aufrufer übersetzen sie in ihren eigenen bestehenden Fehlertyp.
    """
    ...
    logger.warning(
        ...
        "versuche Fallback auf abgeflachtes Schema (KTD5)."
    )
    flat_cls, list_fields, submodels = _build_flat_variant(model_cls)
    flat_schema = flat_cls.model_json_schema()
